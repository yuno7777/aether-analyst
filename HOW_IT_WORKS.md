...
Aether Analyst is deployed as a **split-architecture** production system, with the frontend and backend hosted on independent platforms optimized for their respective workloads.

### Frontend: Vercel (Edge CDN)
*   **Platform**: Vercel auto-deploys the Next.js 16 app from the `main` branch on every `git push`.
*   **Environment Configuration**: The `NEXT_PUBLIC_API_URL` environment variable is set in Vercel's dashboard, pointing the frontend to the backend's HTTPS endpoint. At build time, Next.js inlines this value into the client bundle.
*   **Fallback**: In local development, `API_BASE` falls back to `http://localhost:8000`, so developers can run the full stack locally without any config changes.

### Backend: AWS EC2 + Nginx + SSL
*   **Compute**: An AWS EC2 `t2.micro` instance (Free Tier eligible) runs Ubuntu with a 4GB swap file to handle memory-intensive AI operations (Gemini API calls, pandas data processing, matplotlib chart rendering).
*   **Process Management**: The FastAPI backend runs as a `systemd` service (`aether-backend.service`). Systemd handles:
    - Automatic restart on process crash (`Restart=always`)
    - Boot-time startup (`WantedBy=multi-user.target`)
    - Environment variable loading from `backend/.env` via `EnvironmentFile`
*   **Application Server**: Gunicorn spawns a single Uvicorn worker bound to `0.0.0.0:8000`. A single worker is mandatory because the ReAct agent loop stores in-progress run state in memory — multiple workers would lose context between the initial `POST /api/chat` and subsequent `GET /api/chat/stream` SSE connections.
*   **Reverse Proxy (Nginx)**: Nginx listens on ports 80/443 and proxies all traffic to `http://127.0.0.1:8000`. Critical directives include:
    - `proxy_buffering off` — Required for SSE streaming; without this, Nginx would buffer the event stream and deliver events in batches instead of real-time.
    - `proxy_set_header Connection ''` — Prevents Nginx from injecting `Connection: close` headers that would terminate long-lived SSE connections.
    - `proxy_read_timeout 86400` — Allows SSE connections to persist for up to 24 hours without Nginx forcibly closing them.
*   **SSL/TLS**: Let's Encrypt issues a free certificate via Certbot, with auto-renewal handled by a scheduled cron job. The domain `aether-backend.duckdns.org` is provided by DuckDNS, a free Dynamic DNS service pointing to the EC2 instance's public IP.
*   **CORS**: FastAPI's `CORSMiddleware` is configured with `allow_origins=["*"]` to permit cross-origin requests from the Vercel-hosted frontend. This is essential because the frontend (`https://aether-analyst.vercel.app`) and backend (`https://aether-backend.duckdns.org`) run on different domains.

### The HTTPS Requirement
Vercel serves all content over HTTPS by default. Modern browsers enforce **Mixed Content** security policies, which block HTTP API requests made from HTTPS pages. This is why the EC2 backend must also serve over HTTPS — without SSL, the browser would silently reject every API call from the Vercel frontend, causing the "Backend Offline" indicator despite the server being fully operational.

### Deployment Workflow
1.  **Frontend**: Push to `main` → Vercel auto-builds and deploys within ~60 seconds.
2.  **Backend**: SSH into EC2 → `git pull` → `sudo systemctl restart aether-backend`. The entire backend update cycle takes under 30 seconds.
